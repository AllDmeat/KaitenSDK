...
над SDK — не содержит бизнес-логики, только маппит аргументы
командной строки в вызовы SDK и выводит результат.
...
2. **Given** установлены переменные окружения KAITEN_URL и
   KAITEN_TOKEN, **When** пользователь запускает подкоманду
   без флагов, **Then** CLI использует значения из окружения.
3. **Given** присутствуют и флаги и переменные окружения,
   **When** пользователь указывает `--url` или `--token`,
   **Then** флаги имеют приоритет над переменными окружения.
4. **Given** существует валидный конфиг-файл, **When**
   пользователь запускает подкоманду без флагов и переменных
   окружения, **Then** CLI читает параметры из конфиг-файла.
5. **Given** ни флаги, ни переменные окружения, ни конфиг-файл
   не предоставляют обязательный параметр, **When** пользователь
   запускает подкоманду, **Then** CLI выходит с понятным
   сообщением об ошибке, указывающим какой параметр отсутствует.
6. **Given** SDK возвращает ошибку, **When** пользователь
...
- Что если конфиг-файл существует, но содержит невалидные
  данные? CLI MUST вывести ошибку с описанием проблемы
  конфигурации.
...
- **FR-001**: CLI MUST быть тонкой обёрткой над SDK без
  собственной бизнес-логики — только маппинг аргументов
  командной строки в вызовы SDK и вывод результата.
...
- **FR-003**: CLI MUST резолвить параметры подключения в
  порядке приоритета: флаги командной строки > переменные
  окружения > конфиг-файл.
...
- **SC-001**: CLI можно использовать в скриптах автоматизации —
  ...
  или переменные окружения и выдаёт машиночитаемый вывод.
